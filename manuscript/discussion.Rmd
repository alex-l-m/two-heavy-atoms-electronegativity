...
Although a continuous series of frames were created with incrementally increasing $\Delta \chi$, the value could be obtained from a single frame.
...
From a constrained DFT calculations, the electronegativities can be obtained directly as the Lagrange multipliers of the constraint.
...
In formula \eqref{eq:LinearModel} for $\Delta \chi$, considered as a linear regression model with the partial charges as independent variables, each EEq parameter appears as separately as a regression coefficient.
...
This is reflective of an element of non-locality induced by the total charge constraint, since even atoms that are not in physical contact with each other in a system still compete for fixed pool of charge.
...
However, this is is no different than the cost involved in iteratively solving for zero force, and machine learning of forces has generally been found to be easier than machine learning of final geometries.
...
The we have shown how a dataset including $\chi$ can be constructed, including non-equilibrium electron densities.
Just as forces have been included in many datasets for training machine learning models, it is possible to include electronegativity is as well.
Using constrained density functional theory it is possible to include interesting special cases like a molecules or material in which all atoms are neutral.
The however it may be desirable to include a continuous series of non-equilibrium charges, as we have done here.
...
Although we have not definitely established the advantages of $\chi$-prediction, it is important to raise the question to determine if the next generation of open machine learning datasets should include not just forces and charges, but also electronegativities.
