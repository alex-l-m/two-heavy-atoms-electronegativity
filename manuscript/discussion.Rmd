...
Our simple demonstration opens a new direction in training charge models, in which the model learns a generalized force [@thermo-dynamics] on a charge.

The current generation of machine learning datasets includes atomic $q_k$, as well as non-equilibrium geometries and forces [@isayevani2,@levine2025openmolecules2025omol25].
Charge models would benefit if the next generation includes, not only "non-equilibrium" electron densities obtained by applying a field, but electronegativities $\chi_k$, analogous to the forces included for the non-equilibrium geometries.
These could then be used as a training target for the QEq models used in force fields and machine learning interatomic potentials.
