## Supplementary theory {#sec:supplementary-theory}

### Definition of energy {#sec:definition-of-energy}

We will probe electronegativity by imposing an external electric field that adds a potential $\imposedPotential$.
Let $\hamiltonianNuc$ be the Hamiltonian with no potential imposed (except the potential of the nuclei).
Let $\imposedPotential$ be an additional imposed potential.
Define energies
\begin{equation}
  \energyNuc [ \Psi ] = \left \langle \Psi \middle| \hamiltonianNuc \middle| \Psi \right \rangle
    , \qquad
    \energyTot [ \Psi ] = \left \langle \Psi \middle| \hamiltonianNuc + \imposedPotential \middle| \Psi \right \rangle
  (\#eq:EnergyDefs)
\end{equation}
Then,
\begin{equation}
  \energyTot [ \Psi ] = \energyNuc [ \Psi ] + \spaceIntegral{ \imposedPotentialValue{} \electronDensityValue }
  (\#eq:EnergyTotalSplit)
\end{equation}
Considering both energies as density functionals $\energyNuc [ \electronDensity ]$ and $\energyTot [ \electronDensity ]$, this becomes
\begin{equation}
  \energyTot [ \electronDensity ] = \energyNuc [ \electronDensity ] + \spaceIntegral{ \imposedPotentialValue{} \electronDensityValue }
  (\#eq:EnergyTotalSplitDensityFunctional)
\end{equation}

### Functional derivative of energy {#sec:functional-derivative-of-energy}

$\energyTot$ is stationary with respect to variations in the density:
...
Beginning with equation \@ref(eq:EnergyTotalSplitDensityFunctional), taking the functional derivative with respect to the density yields
\begin{equation}
...
### Definition of atomic electron populations and charges {#sec:atomic-populations-and-charges}
...
### Definition of atomic electronegativity {#sec:atomic-electronegativity}
...
In terms of charge, equation \@ref(eq:ChemicalPotentialDef) becomes
\begin{equation}
  \chi_k
...
### Measuring atomic electronegativity by imposing a potential {#sec:measuring-atomic-electronegativity}
...
Substituting Equation \@ref(eq:ImposedPotential) into Equation \@ref(eq:RelateNucPotential) gives
...
#### Special case: fixed weight functions {#sec:fixed-weight-functions}

If these are Hirshfeld weight functions according to the original scheme with fixed neutral proatoms, then we have:
...
This is all that is required for a constant weight function such as in the original Hirshfeld method [@originalHirshfeld], and is typically referred to as a "thermodynamic relation" in the context of constrained DFT [@KadukConstrainedDFTReview].
...
#### Accounting for the shift in the atom boundary {#sec:accounting-for-boundary-shift}
...
#### Special case for two atoms {#sec:two-atom-moving-boundary}
...
\begin{equation}
  ...
  (\#eq:TwoAtomAvgWeightDerivRelation)
\end{equation}
...
This is also the electronegativity difference $\Delta \chi = \chi_A - \chi_D$.

### Linear regression model for atomic electronegativities {#sec:linear-regression-electronegativities}
...
$c_{ij}$ is a symmetric ($c_{ij}=c_{ji}$) interaction term, assumed to have some explicit functional form as a function of the distance between the atoms, whereas $\chi_i^0$ and $\eta_i$ depend on empirical parameters of the model.
...
#### Special case for two atoms {#sec:two-atom-regression}
...
\begin{equation}
  \Delta \chi
    = \chi_{e(A)}^0 - \chi_{e(D)}^0
      + \eta_{e(A)} \, q_A - \eta_{e(D)} \, q_D
  (\#eq:DeltaLambdaLinearModel)
\end{equation}
where $e(D)$ is the donor element and $e(A)$ is the acceptor element.

#### Example {#sec:linear-regression-example}
...
#### Zero of the electronegativity scale in regression {#sec:zero-of-electronegativity-scale}

In charge equilibration, electronegativities $\chi^0$ are considered as differences from an arbitrary reference electronegativity.
...
#### Primitive Hirshfeld-I {#sec:primitive-hirshfeld-i}
...
The main issue with the Hirshfeld-I method is anions with unbound or nearly unbound electrons, with the nitrogen anion causing particular trouble [@fractionalChargeHirshfeldI].
