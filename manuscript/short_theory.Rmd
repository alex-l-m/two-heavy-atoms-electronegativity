...
Here we will use electron populations defined as integrals of a weight function $\weightFunc{k}{\vect{N}}$ against the density function:
\begin{equation}
...
We will perform simulations with an external electrical potential imposed.
In order to relate the electrical potential to atomic electronegativities, the field must have a form defined by the weight functions:
\begin{equation}
...
We impose a shaped electrical potential such that $\lambda_A = -\lambda_D = \lambda$ so that
\begin{equation}
...
To define electronegativities, we consider the energy $\energyNuc$ (defined formally in the Supplementary Theory), which includes the Coulomb potential of the nuclei but not the imposed electrical potential.
...
Following previous research which trained QEq models by minimizing squared error in charge prediction, and considering that we examine only binary systems with zero total charge, we could train QEq by minimizing squared error in prediction of charge of the acceptor atom:
\begin{equation}
\sum_i \left(\hat{q}_A^{(i)} - q_A^{(i)}\right)^2
(\#eq:ChargeLossSqSummary)
\end{equation}
where $i$ indexes over the frames simulated.
Instead, we train the model by minimizing the loss function
\begin{equation}
...
The term "generalized force" has been used for derivatives of energy with respect to a constraint [@thermo-dynamics], and the perspective of constrained DFT reveals atomic electrongativities to be generalized forces.
...
<!-- Hohenberg-Kohn map -->
It is also worth noting that since the $q_k$ are computed from the electron density and the $\lambda_k$ parametrize the potential, the $\chi$-prediction task requires learning a map from the density to the potential.
