...
In the Supplementary Theory all results are derived for an arbitrary number of atoms and specialized for the case of two atoms.
In what follows we give the formulas without derivations, in the two-atom special case where appropriate.
...
In our simulations we manipulate charge by imposing an external potential.
To define electronegativities, we consider the energy $\energyNuc$ (defined formally in equation \@ref(eq:EnergyDefs)), which includes the Coulomb potential of the nuclei but not the imposed potential.
...
(\#eq:ImposedPotentialSummary)
\end{equation}

For our two-atom systems, we impose a shaped potential such that $\lambda_A = -\lambda_D = \lambda$ so that
\begin{equation}
...
This potential is positive on the acceptor to push electron density back onto the donor, but interpreted as an electrical potential, it is electrically negative on the acceptor to repel electrons back to the donor.
...
\end{equation}
where $\avgWeightDerivSubscript{A}{A}{N}$ is the derivative of the weight function integrated against the electron density, defined fully in equation \@ref(eq:TwoAtomAvgWeightDerivRelation).
We evaluate this factor by numerical integration.

<!-- Constrained density functional theory -->
In constrained density functional theory, the penalty term enforcing the charge constraint can be interpreted as a potential of the form in Equation \@ref(eq:ImposedPotentialSummary), and Equation \@ref(eq:DeltaChiNoBoundarySummary) follows from a "thermodynamic relation" between the Lagrange multiplier and the chemical potential [@KadukConstrainedDFTReview].
...
The linear model underlying QEq, in the two-atom case, is
\begin{equation}
...
This model does not include the Coulomb interaction term typically included in QEq models, but our Supplementary Theory treats the general case with interaction terms.

Charge is predicted by electronegativity equalization, that is, setting $\widehat{\Delta \chi}=0$ and $q_D+q_A=0$ in Equation \@ref(eq:QEqLinearModelSummary), which after solving for $q_A$ yields
\begin{equation}
