...
It is therefore an urgent question how to best construct datasets like these.

Another potentially important property besides force is partial charge.
In both polarizable force fields [@polarizableFFReviewJing] and some MLIPs [@Anstine2023], energy is partitioned into a short-range component, and a long-range electrostatic component which is computed using charges predicted from the geometry.
In classical force fields that predict charge it is usually predicted using a variant of charge equilibration (QEq), and the idea has been revived in "fourth-generation" MLIPs [@fourGenerations].
...
The familiar Mulliken electronegativity [@originalMulliken] can be obtained by replacing a derivative with a difference.
...
We will use the term "electronegativity equalization" to refer to predicting charge by solving this equation.
