...
QEq begins by assigning each atom $k$ an electronegativity $\chi^0_k$ and a hardness $\eta_k$, and using these parameters, solves a linear system to obtain atomic charges $q_k$.
...
Defined this way, it is an increasing function of charge, $\chi(q)$.
For example, the fluorine atom is very electronegative (high $\chi(0)$), but the fluorine anion less so (smaller $\chi(-1)$).
A discrete difference approximation to $\chi(0)$ yields the familiar Mulliken electronegativity.
...
QEq is based on a modeling assumption that the atomic electronegativities $\chi_k(q)$ vary linearly with the charge $q$, with $\chi^0_k$ as the intercept and the hardness $\eta_k$ as the slope.
