...
For calculations beyond the computational range of density functional theory (DFT), we use force fields, which achieve speed but sacrifice accuracy.
More recently, machine learning interatomic potentials (MLIPs) promise to approach DFT accuracy, but first must be trained on large DFT datasets [@Anstine2023].
...
Charge equilibration (QEq) [@rappe1991charge] is typically used for assigning charges to atoms in classical force fields, and the idea has been revived in "fourth-generation" MLIPs [@fourGenerations].

Some datasets also include the charges of each atom [@comp6,@omol25].
Interestingly, training of classical QEq models has sometimes included systems perturbed by potentials such as the electric field from a nearby point charge [@verstraelenElectronegativityParameters,@verstraelenSilicatesResponse], though ML-scale datasets do not currently include such perturbations.

Our purpose in this work will be to propose not only the inclusion of perturbing potentials in MLIP training datasets, but to annotate these perturbed systems with an additional atomic property: electronegativity.
...
QEq requires not only the electronegativity $\chi$ of a system, but an analogous definition for the electronegativity $\chi_k$ of an atom $k$ within the system [@conceptualDFTElectronegativityOriginal].
...
QEq methods predict charge by electronegativity equalization: that is, solving for the atomic charges $\vect{q}$ such that all atomic electronegativities are equal, under a constraint on total charge.
...
The parameters of classical QEq methods are two free parameters per element $e$: $\chi^0_e$, the electronegativity of a neutral atom of that element, and a hardness $\eta_e$ that describes how the electronegativity increases with charge.
...
That is, the parameters of the model, whether the element-specific $\chi^0_e$ and $\eta_e$ values of a classical QEq model or the much larger number of parameters used by a modern ML model to assign environment-dependent $\chi^0_k$ and $\eta_k$, are determined by minimizing prediction error in a training dataset.
