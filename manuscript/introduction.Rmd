...
The frontiers of computational chemistry are simulation of larger molecules, longer timescales in molecular dynamics, and larger combinatorial spaces of structures.
...
The loss function is the error in charge prediction on a training dataset [@verstraelenElectronegativityParameters].

While it may seem inevitable that charge models are trained by minimizing charge prediction error, it is interesting to contrast to another prediction task, prediction of equilibrium geometries.
...
This suggests an alternative idea for training charge prediction models, based on a mathematical analogy.
The force is the negative derivative of energy with respect to nuclear position.
If we instead take the negative derivative with respect to the electron population of the atom, what we have is the definition of electronegativity used in conceptual density functional theory [@conceptualDFTElectronegativityOriginal].
...
A model that maps charges to electronegativities may seem useless because the charges are not known.
However, charge can be estimated by solving for the charges that equalize the electronegativity of each atom.
...
We will call electronegativity prediction $\chi$-prediction, in contrast to directly predicting charge, which we will refer to as $q$-prediction.
...
$q$-prediction by electronegativity equalization can be accomplished by iterative root-finding methods such as Newton's method.
Since the term "electronegativity equalization" refers to the classical linear method, this could be regarded as generalized electronegativity equalization.

However, even though EEq can be considered a linear $\chi$-prediction model, it is not trained on $\chi$-prediction task.
However, just as forces tend to be easier to learn than equilibrium geometries, is worth considering that electronegativities may be more learnable than charges.
First of all, by providing non-equilibrium electron densities, it is possible to expand the dataset in the same way as non-equilibrium geometries expand the dataset used to train a force model.
