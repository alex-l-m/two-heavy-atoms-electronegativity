...
Using external potentials to augment ML training datasets with distorted electron densities could help models generalize to chemical environments not directly present in the training data, or learn the polarization required for QM/MM [@polarizableQMMM].

<!-- 2. Conceptual DFT / Electronegativity is a real thing -->
However, our purpose in this work is to propose not just distorting electron densities with potentials, but also annotating atoms with a property associated with a distorted electron density just as force is associated with a distorted geometry.
...
We train in what we call the "$\chi$-prediction task", using the model to predict $\chi_k$ for each atom, and minimizing the error of these predictions on the training dataset.
Although QEq has a natural framing as a linear model for $\chi_k$, our study is the first to train a QEq model with $\chi_k$ values.
Once the parameters of the QEq model have been obtained in the $\chi$-prediction task, we use the resulting model for charge prediction (the $q$-prediction task) by equalizing electronegativity.
