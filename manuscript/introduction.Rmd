...
Over the past decade, machine learning interatomic potentials (MLIPs) have aimed to approach DFT-level accuracy with force-field efficiency, but training them requires large datasets generated from *ab initio* calculations [@MILPReview2025].
...
Notably, some prior work has augmented this training by including systems subjected to external perturbations—such as the electrostatic potential created by a nearby point charge—to probe how charges respond under imposed fields [@verstraelenElectronegativityParameters; @verstraelenSilicatesResponse].
...
The possibility we will explore in this work is not just distorting electron densities via external potentials, but annotating the data with an new atomic property, associated with distorted densities just as force is associated with distorted geometries.
...
To demonstrate this, we train a QEq model on a dataset of binary materials, determining its parameters of by minimizing the error in predicting $\chi_k$.
