...
There is thus a tension between its physical basis as a model for $\chi$, the training procedure by minimizing error in predicting $q$.
...
Train on the $\chi$-prediction task requires a dataset with not only atomic charges $q_k$ but atomic electronegativity $\chi_k$ for each atom.
...
It would not be as simple as solving the linear system from QEq, but could still be accomplished by iterative root-finding methods such as Newton's method.
